...
#
start = time.clock()
cores = np.array([0, 201, 1955, 1435])
fsfdp_classification = FSFDP(three_data, t=0.0005, distance_func=distance_calc, gama_graph=True, cluster_cores=cores)
...
fig = plt.figure()
ax = Axes3D(fig)
ax.scatter(three_data[:, 0], three_data[:, 1], three_data[:, 2], c=fsfdp_classification)
...
# ---- KMeans ----
start = time.clock()
km_class_result = KMeans(n_clusters=4, random_state=9).fit_predict(three_data)
elapsed = (time.clock() - start)
print("KMeans time used ", elapsed)
print(km_class_result.shape)
fig = plt.figure()
ax = Axes3D(fig)
ax.scatter(three_data[:, 0], three_data[:, 1], three_data[:, 2], c=km_class_result)
...
# ---- DBSCAN ----
start = time.clock()
db_classification_result = DBSCAN(eps=0.09).fit_predict(three_data)
elapsed = (time.clock() - start)
...
fig = plt.figure()
ax = Axes3D(fig)
ax.scatter(three_data[:, 0], three_data[:, 1], three_data[:, 2], c=db_classification_result)
ax.set_zlabel('Z', fontdict={'size': 10, 'color': 'red'})
ax.set_ylabel('Y', fontdict={'size': 10, 'color': 'red'})
ax.set_xlabel('X', fontdict={'size': 10, 'color': 'red'})
ax.view_init(elev=30, azim=20)
